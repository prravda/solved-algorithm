...
class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        # sort position in DESC order
        position_desc_sorted = sorted(position, reverse=True)

        # re-arrange speed too
        # should be enhanced
        speed_followed_sorted_with_position = []
        for pds in position_desc_sorted:
            pds_index_in_original = position.index(pds)
            speed_followed_sorted_with_position.append(speed[pds_index_in_original])
